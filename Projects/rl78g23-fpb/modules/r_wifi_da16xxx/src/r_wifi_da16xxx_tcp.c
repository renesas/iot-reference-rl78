 ...
 *                WIFI_ERR_TAKE_MUTEX
 *********************************************************************************************************************/
wifi_err_t R_WIFI_DA16XXX_TcpConnect (uint8_t socket_number, uint8_t * ip_address, uint16_t port)
{
    ...
    if (AT_OK == at_exec_wo_mutex("AT+TRTC=%d.%d.%d.%d,%hu,0\r",
                                  ip_address[0], ip_address[1], ip_address[2], ip_address[3], port))
    {
        if (DATA_FOUND == at_read("+TRTC:%hhu", &cid))
        {
                                   ...
                                   uint16_t length, uint32_t timeout_ms)
{
    ...
    WIFI_LOG_INFO(("R_WIFI_DA16XXX_SendSocket: socket %d ret=%d (%lu).", socket_number, api_ret, tick_tmp));
    return api_ret;
}
/**********************************************************************************************************************
                                      ...
                                      uint16_t length, uint32_t timeout_ms)
{
    ...
    WIFI_LOG_INFO(("R_WIFI_DA16XXX_ReceiveSocket: socket %d recv_cnt=%d (%lu).", socket_number, recv_cnt, tick_tmp));
    api_ret = recv_cnt;
    return api_ret;
}
/**********************************************************************************************************************
